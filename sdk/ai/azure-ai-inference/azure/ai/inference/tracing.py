...
class AIInferenceInstrumentorPreview:
    ...
    def _add_request_chat_message_event(
        self, span: "AbstractSpan", **kwargs: Any
    ) -> None:
    ...
    def _add_request_chat_attributes(
        self, span: "AbstractSpan", *args: Any, **kwargs: Any
    ) -> None:
        client = args[0]
    ...
    def _add_response_chat_message_event(
        self, span: "AbstractSpan", result: _models.ChatCompletions
    ) -> None:
        for choice in result.choices:
            if _trace_inference_content:
                ...
                if choice.message.tool_calls:
                    full_response["message"]["tool_calls"] = [
                        tool.as_dict() for tool in choice.message.tool_calls
                    ]
                attributes = {
                    "gen_ai.system": INFERENCE_GEN_AI_SYSTEM_NAME,
                ...
                if choice.message.tool_calls:
                    ...
                    tool_calls_function_names_and_arguments_removed = (
                        self._remove_function_call_names_and_arguments(
                            choice.message.tool_calls
                        )
                    )
                    response["message"]["tool_calls"] = [
                        tool.as_dict()
                        for tool in tool_calls_function_names_and_arguments_removed
                    ]
    ...
    def _add_request_span_attributes(
        self, span: "AbstractSpan", _span_name: str, args: Any, kwargs: Any
    ) -> None:
        self._add_request_chat_attributes(span, *args, **kwargs)
        if _trace_inference_content:
            self._add_request_chat_message_event(span, **kwargs)

    def _add_response_span_attributes(
        self, span: "AbstractSpan", result: object
    ) -> None:
            ...
            if item.delta.tool_calls is not None:
                for tool_call in item.delta.tool_calls:
                    ...
                    if tool_call.function and tool_call.function.name:
                        accumulate["message"]["tool_calls"][-1]["function"][
                            "name"
        ...
        class StreamWrapper(_models.StreamingChatCompletions):
            ...
            def __iter__(  # pyright: ignore [reportIncompatibleMethodOverride]
                ...
                try:
                    ...
                    module = (
                        exc.__module__
                        if hasattr(exc, "__module__") and exc.__module__ != "builtins"
                        else ""
                    )
                    error_type = (
                        f"{module}.{type(exc).__name__}"
                        if module
                        ...
                        if not _trace_inference_content:
                            ...
                            if "message" in accumulate:
                                if "tool_calls" in accumulate["message"]:
                                    tool_calls_function_names_and_arguments_removed = self._instrumentor._remove_function_call_names_and_arguments(
